...
namespace Impostor.Plugins.EBPlugin.Handlers
{
    public class GameEventListener : IEventListener
    {
        ...
        public struct CRoleEnable
        {
            ...
            public Boolean RandomThirdEnabled;
        ...
        */
        ...
        public void GameStarted(IGameStartedEvent e)
        {
            ...
            //##RandomThirdランダム化##
            if(CRE.RandomThirdEnabled) {
                var ToMRand = rand.Next(3);
                if(ToMRand == 0) {
            ...
            //##Clone##
            if(CRE.CloneEnabled) {
                var CloneNum = rand.Next(Everyone.Count);
                CRID.CloneID = Everyone[CloneNum].Client.Id;
                var Clone = Everyone[CloneNum];
                Everyone.Remove(Everyone[CloneNum]);
            }
            //##リーダー##
            ...
            //Cloneの見た目を変更
            ChangeCloneCosmetics(e.Game);
            //inCommsSabotageをリセット
            inCommsSabotage = false;
            //ターゲット指定
        ...
        public void PlayerVotedOut(IPlayerExileEvent e)
        {
            ...
            if(CRE.CloneEnabled && e.ClientPlayer.Client.Id == CRID.ToCloneID) ChangeCloneCosmetics(e.Game);
        }
        [EventListener]
        public void Commands(IPlayerChatEvent e)
        {
            ...
            if(e.Message.StartsWith("/")){
                ...
                if(cmd1 == "clone") {
                    if(e.Game.GameState == 0) {
                        if(cmd2 == "on") {
                            PlayerCTRL.SendChatToPlayerAsync("Cloneが有効化されました。");
                            CRE.CloneEnabled = true;
                        } else if(cmd2 == "off") {
                            PlayerCTRL.SendChatToPlayerAsync("Cloneが無効化されました。");
                            CRE.CloneEnabled = false;
                        } else {
                            PlayerCTRL.SendChatToPlayerAsync("エラー:2つ目の引数が無効です。\r\non/offで指定してください。");
                        }
                    } else {
        ...
        public void BodyReportedEvent(IPlayerReportEvent e) {
            ...
            if(CRE.HideAndSeekEnabled) e.IsCancelled = true;
        }
        [EventListener]
        ...
        public void ChangeCloneCosmetics(Api.Games.IGame Game) {
            if(CRE.CloneEnabled) {
                var Clone = Game.GetClientPlayer(CRID.CloneID);
                if(!Clone.Character.PlayerInfo.IsDead) {
                    var Clonable = new List<Api.Net.IClientPlayer>();
                    foreach(var p in Game.Players) {
                        if(!p.Character.PlayerInfo.IsDead &&
                        p.Client.Id != CRID.CloneID) 
                            Clonable.Add(p);
                    }
                    var ToClone = Clonable[rand.Next(Clonable.Count)];
                    CRID.ToCloneID = ToClone.Client.Id;
                    var TCI = ToClone.Character.PlayerInfo;
                    Clone.Character.SetColorAsync(TCI.Color);
                    Clone.Character.SetHatAsync(TCI.Hat);
                    Clone.Character.SetNameAsync(TCI.PlayerName);
                    Clone.Character.SetSkinAsync(TCI.Skin);
                    Clone.Character.SetPetAsync(TCI.Pet);
                    names[Clone.Character.PlayerId] = Clone.Character.PlayerInfo.PlayerName;
                }
            }
        }
        [EventListener]
        ...
        [EventListener]
        public void SabotageEvent(IPlayerRepairSystemEvent e) {
            _logger.LogInformation("ID:" + ((byte)e.SystemType));
            _logger.LogInformation("Amount:" + e.Amount.ToString());
            if(e.SystemType == SystemTypes.Sabotage && e.Amount == 14)
                _logger.LogInformation("コミュサボ発生");
            if(e.SystemType == SystemTypes.Comms)
                _logger.LogInformation("コミュサボ修復");
        }
        [EventListener]
        public void WhenKilled(IPlayerMurderEvent e) {
            CheckWinner(e.Game);
            _logger.LogInformation("キル発生時の処理開始");
            if(!CRE.FastMeetingStarted) {
                foreach(var player in e.Game.Players) {
            ...
            _logger.LogInformation("完了:FastTroll処理");
            foreach(var player in e.Game.Players) {
                ...
                _logger.LogInformation("完了:Stopper処理");
                ...
                _logger.LogInformation("完了:MagnetTroll処理");
            }
            if(CRE.HideAndSeekEnabled) {
                /*float bodyTP = -64;
            ...
            _logger.LogInformation("完了:HideAndSeek用処理");
            //TargetMode用の処理
            if(CRE.TargetModeEnabled) {
                Impostor.Api.Net.IClientPlayer Target = null;
            ...
            _logger.LogInformation("完了:TargetMode用処理");
            ...
            _logger.LogInformation("完了:TheBlackOut用処理");
            if(CRE.LeaderEnabled && e.Victim.PlayerId == e.Game.GetClientPlayer(CRID.LeaderID).Character.PlayerId) {
                LeaderDead(e.Game, e.Game.GetClientPlayer(CRID.LeaderID), e.ClientPlayer);
                CheckWinner(e.Game);
            }
            _logger.LogInformation("完了:リーダー用処理");
            if(CRE.CloneEnabled && e.Victim.PlayerId == 
            e.Game.GetClientPlayer(CRID.ToCloneID).Character.PlayerId) ChangeCloneCosmetics(e.Game);
            _logger.LogInformation("完了:Clone用処理");
        }
        [EventListener]
